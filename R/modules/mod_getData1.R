...
getData<-function(input, output, session){
  ...
  ## priority set after sheets() so won't throw an error at start when there is no input$userFile
  observe(priority = 10, {

    req(input$dataset)

    dataHere$sampleData<-switch(input$dataset,
                                "mtcars"=mtcars,
                                "iris"=iris)
  })
